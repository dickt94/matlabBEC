function [samples,times]=npw_adiabaticramp()
...
%     end
    ...
    %GC sample of a non-interacting thermal state
    
    n0=gpuArray([1000;zeros(nmodes-10,1);1000;zeros(8,1)]);
    phi0=2*pi*rand(nmodes,npaths);
    c0=gpuArray(zeros([nmodes+1 npaths]));
    ...
    function F=f(c,t)
        ...
        %adiabatically switch interactions
        g1eff=g_1D*(t/time_int);
        F=F-[(g1eff)*(1i)*invtrans_4f*(w_4f.*(abs(psi).^2.*psi-delt_4f.*psi));dumw];%-delt_4f.*psi
...
%         m_pjb=m_pj;
    ...
    [samples,times]=rk4int(c0,@f,ipevol,true,0,{},{},[],0,time_int,nsteps,...
