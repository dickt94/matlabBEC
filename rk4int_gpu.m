...
function [sdata,tdata]=rk4int_gpu(ci1,ci2,f1,f2,ipop1,ipop2,ipdiag1,ipdiag2,nproc,g1,g2,nnoise,seed,ti,tf,nsteps,moments,samples,filters,errorcheck)
    ...
    for count=1:nproc
        ...
        streams{count}=parallel.gpu.RandStream.create('Philox4x32-10', 'NumStreams',1,'Seed',seed(count));
    end
    ...
    while stepcount < nsteps
        dW=cell(nproc,1);
        for count=1:nproc
            dW1=sigma*gpuArray.randn(streams{count},[nnoise{count} 1]);
            dW2=sigma*gpuArray.randn(streams{count},[nnoise{count} 1]);%roll twice - this keeps the seeding the same for halfstep
    ...
    if errorcheck
        ...
        for count=1:nproc
            %create a random stream with the specified seed
            streams{count}=parallel.gpu.RandStream.create('dsfmt19937', 'NumStreams',1,'Seed',seed(count));
        end
        ...
        while stepcount<nsteps*2
            ...
            dW=cell(nproc,1);
            for count=1:nproc
                dW{count}=sigma*gpuArray.randn(streams{count},[nnoise{count} 1]);
            end
