...
%     end
    ...
    g_1D = 0;%0.1;%0.4;%2.0 * Lambda * a_s / sqrt( hbar / (m * omega_x) );
    ...
    function F=ffil(csys,c,~)
        ...
        w=exp(c(end,:)).';
        w=w/sum(w);
        norm=sum((conj(c(1:end-1,:)).*c(1:end-1,:)-0.5*ones([nmodes npaths],'gpuArray'))*w);
        %nl feedback
        ...
        %ep=0;
        %flat= @(M) M(:);
        ep=2/norm*sqrtn12.'*imag(conj(c(1:end-1,:)).*[c(2:end-1,:);zeros([1 npaths],'gpuArray')])*w;
        %ep=2/norm*sum(flat(w.'.*conj(c(1:end-1,:)).*[c(2:end-1,:);zeros([1 npaths])].*sqrtn12));        
        xppx=2/norm*(sqrtnp1.*sqrtnp2).'*imag(conj(c(1:end-1,:)).*[c(3:end-1,:);zeros([2 npaths],'gpuArray')])*w;
        ...
        %add feedback
        F=F-1i*[fbsl*ep*(sqrtn2.*[zeros([1 npaths],'gpuArray'); c(1:end-2,:)]+sqrtn12.*[c(2:end-1,:);zeros([1 npaths],'gpuArray')])+fbbr*xppx*(0.5*(sqrtn.*sqrtnm1.*[zeros([2 npaths],'gpuArray'); c(1:end-3,:)]+sqrtnp1.*sqrtnp2.*[c(3:end-1,:);zeros([2 npaths],'gpuArray')])+(nx+0.5).*c(1:end-1,:));dumw];
        
        %energy-damping feedback
        %compute energy-damping feedback potential
        ved=imag(sum(w.'.*conj(psi).*(trans_4f*(0.5*(sqrtnp1.*sqrtnp2.*[c(3:end-1,:);zeros([2 npaths],'gpuArray')]+sqrtn.*sqrtnm1.*[zeros([2 npaths],'gpuArray');c(1:end-3,:)])-(nx+0.5).*c(1:end-1,:))),2));
...
%         m_pjb=m_pj;
        ...
        mj2=diag(m_pj).';
        wmj2=sum(w.'.*mj2);
        detw=+2*eta*alpha*(-(mj2-wmj2));
        
        %add the deterministic bit of the measurement signal
        wmsmj=sum(wsys.*m_spj,1);
        wmswm=sum(wsys.'.*wmsmj);
        detw=detw+4*eta*alpha*(wmsmj-wmswm);%should always converge, mult by alpha for real update
    ...
    function F=fsys(c,cfil,~)
        ...
        wfil=exp(cfil(end,:)).';
        wfil=wfil/sum(wfil);
        normfil=sum((conj(cfil(1:end-1,:)).*cfil(1:end-1,:)-0.5*ones([nmodes npaths],'gpuArray'))*wfil);
        %nl feedback
        %ufb=-sum(w_4f.*imag(sum(w.'.*conj(psi).*abs(psi).^2.*(trans_4f*(0.5*(sqrtnp1.*sqrtnp2.*[c(3:end-1,:);zeros([2 npaths])]+...
        %    sqrtn.*sqrtnm1.*[zeros([2 npaths]);c(1:end-3,:)])-(nx+0.5).*c(1:end-1,:))),2)))/(norm^2);
        %wigner correction added 30/08
        ...
        %ep=0;
        %flat= @(M) M(:);
        ep=2/normfil*sqrtn12.'*imag(conj(cfil(1:end-1,:)).*[cfil(2:end-1,:);zeros([1 npaths],'gpuArray')])*wfil;
        %ep=2/norm*sum(flat(w.'.*conj(c(1:end-1,:)).*[c(2:end-1,:);zeros([1 npaths])].*sqrtn12));        
        xppx=2/normfil*(sqrtnp1.*sqrtnp2).'*imag(conj(cfil(1:end-1,:)).*[cfil(3:end-1,:);zeros([2 npaths],'gpuArray')])*wfil;
        ...
        %energy-damping feedback
        %compute energy-damping feedback potential
        ved=imag(sum(wfil.'.*conj(psifil).*(trans_4f*(0.5*(sqrtnp1.*sqrtnp2.*[cfil(3:end-1,:);zeros([2 npaths],'gpuArray')]+sqrtn.*sqrtnm1.*[zeros([2 npaths],'gpuArray');cfil(1:end-3,:)])-(nx+0.5).*cfil(1:end-1,:))),2));
...
%     end
    ...
    function corr=filtercorr(c1,c2,~)
        ...
        wmsmj=sum(w1.*m_spj,1);
        wmswm=sum(w1.'.*wmsmj);
        
        dlogwf=4*sqrte*alpha*(wmsmj-wmswm-wmmj+wmwm);
        dwf=dlogwf.*w2;
        
        psi_2f=trans_2f*c2(1:end-1,:);
    ...
    function diff=densdiff(cs,cf,~)
        ...
        wf=exp(cf(end,:)).';
        wf=wf/sum(wf);
        psif_2f=trans_2f*cs(1:end-1,:);
        psis_2f=trans_2f*cf(1:end-1,:);
        diff=sum(w_2f.*abs((conj(psif_2f).*psif_2f)*wf-(conj(psis_2f).*psis_2f)*ws));
    end
...
%    end
    ...
    ipevol=0;%[-(1i)*(nx+0.5); 0];

    seed1=mod(randi([0 nsteps*nmodes*npaths]),2^32);
    seed2=mod(randi([0 nsteps*nmodes*npaths]),2^32);
    seed3=mod(randi([0 nsteps*nmodes*npaths]),2^32);
    
    dumfunc=@(~,~,~,~) 0;
    
    [samples,times]=rk4int_gpu(c01,c02,@fsys,@ffil,ipevol,ipevol,true,true,3,{fieldnoise1 dumfunc weightevol1},{dumfunc fieldnoise2 weightevol2},{[nmodes npaths],[nmodes npaths],[nmodes 1]},[seed1 seed2 seed3],0,time_int,nsteps,...
        {normS,normF,eS,eF,rhoS,rhoF,rhoxS,rhoxF,evenS,evenF,mvarS,mvarF,fsS,fsF,@meassig,@fbdiff,@veddiff,@filtermoments,@sysmoments,@filtercorr,@densdiff},...
        [500 500 500 500 100 100 100 100 500 500 500 500 1 1 100 500 100 500 500 100 500],{@breed},false);
